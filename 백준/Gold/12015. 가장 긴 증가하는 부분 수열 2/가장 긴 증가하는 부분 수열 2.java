...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (answer.length == 0 || answer[index] < num) { // num이 현재 가장 긴 부분수열의 최대값보다 크면, index를 하나 늘려주고 그 자리에 num을 추가함
                answer[++index] = num;
            } else if (answer[index] > num) { // num이 부분수열의 최대값보다 같거나 작으면, 알맞은 자리를 이진 탐색을 통해 찾아줌
